...
쇼핑몰 프론트엔드에서 필요한 요청/응답 구조, 인증 흐름, 주문 상태 변경 기준을 이해하기 위해 구현했습니다.

외부 PG 결제와 택배사 API 연동은 제외하고, 무통장 입금과 관리자 배송 처리 방식으로 주문 이후 운영 흐름을 구성했습니다.
...
쇼핑몰 백엔드는 상품 CRUD보다 주문 상태를 어떻게 일관되게 관리할지가 더 중요했습니다.

프론트엔드에서는 사용자가 상품을 선택하고 주문을 생성하지만, 서버에서는 주문 시점의 옵션 조합과 재고를 검증해야 했습니다.  
또한 주문이 생성된 뒤에는 입금 확인, 배송 처리, 주문 취소, 반품 요청, 환불 처리처럼 여러 상태 변화가 이어집니다.

처음에는 실제 결제 PG와 택배사 API까지 붙여야 하는지 고민했습니다.  
하지만 프로젝트 범위상 외부 결제/배송 연동까지 포함하면 주문 상태와 API 흐름을 정리하기 전에 구현 범위가 너무 커진다고 판단했습니다.

그래서 결제는 무통장 입금 방식으로 두고, 관리자가 입금 확인 후 배송 상태를 변경하는 흐름으로 구성했습니다.  
실제 배송은 우체국 직접 발송을 가정하고, 백엔드에서는 배송 상태 변경과 주문 상태 전환을 관리하는 데 집중했습니다.

비회원 주문도 중요한 문제였습니다.  
비회원은 계정 정보가 없기 때문에 주문번호와 주문 시 입력한 휴대폰 번호를 기준으로 이후 조회, 취소 요청, 반품 요청을 처리하도록 구성했습니다.

이 프로젝트에서는 인증 정책, 주문 생성 정책, 재고 차감/복구, 반품/환불 상태 전환을 백엔드 기준으로 정리하는 것을 핵심 문제로 잡았습니다.
...
실제 카드 결제 PG와 택배사 API 연동은 1차 프로젝트 범위에서 제외했습니다.  
대신 사용자가 주문을 생성하면 입금 대기 상태가 되고, 관리자가 입금 확인과 배송 처리를 진행하는 방식으로 주문 상태를 관리했습니다.

배송은 우체국 직접 발송을 가정하고, 서비스 안에서는 배송 상태를 `SHIPPED`, `DELIVERED`처럼 변경하는 흐름에 집중했습니다.

주문이 생성되면 `AWAITING_DEPOSIT` 상태가 되고, 관리자가 입금 확인을 하면 `DEPOSIT_CONFIRMED` 상태로 변경됩니다.  
이후 배송 정보를 등록하면 `SHIPPED`, 구매 확정 또는 일정 기간 이후 `DELIVERED` 상태로 변경됩니다.
...
| Result | 외부 결제/배송 API 없이도 주문 생성, 입금 확인, 배송 처리, 배송 완료, 취소 요청, 반품 요청으로 이어지는 쇼핑몰 운영 흐름을 구현할 수 있었습니다. |
