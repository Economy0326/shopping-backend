...
### 3. 회원 / 비회원 주문 흐름을 함께 처리해야 하는 문제

| 항목 | 내용 |
| --- | --- |
| Problem | 회원 주문과 비회원 주문은 사용자 식별 방식이 다르지만, 주문 생성과 재고 차감 정책은 동일하게 유지되어야 했습니다. |
| Cause | 회원은 Access Token 기준으로 userId를 연결할 수 있지만, 비회원은 주문자 정보와 휴대폰 번호를 기준으로 주문 조회와 처리 권한을 확인해야 했습니다. |
| Fix | 주문 생성 API는 Optional JWT 인증을 사용해 회원/비회원 모두 접근 가능하게 하고, 주문 상세/취소/반품 요청은 회원 또는 비회원 phone 검증 기준으로 처리했습니다. |
| Result | 로그인하지 않은 사용자도 주문할 수 있으면서, 회원 주문과 동일한 옵션 검증, 재고 차감, 주문 상태 정책을 적용할 수 있게 되었습니다. |
