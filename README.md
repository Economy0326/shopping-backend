...
프론트엔드와 맞춰 주문 생성, 비회원 주문 조회, 주문 상태 변경, 반품 처리 흐름을 확인할 수 있도록 만들었습니다.

외부 PG 결제와 택배사 API 연동은 제외했습니다.  
결제는 무통장 입금 흐름으로 두고, 배송은 관리자가 입금 확인과 배송 처리를 진행하는 방식입니다.
...
이 프로젝트에서 가장 중요하게 본 부분은 주문 이후의 상태 흐름입니다.

상품 조회나 장바구니 흐름은 프론트엔드에서 시작되지만, 주문이 생성되는 순간부터는 서버가 옵션 조합, 재고, 주문 상태를 일관되게 검증해야 합니다.
...
덕분에 주문 생성, 입금 확인, 배송 처리, 배송 완료, 취소 요청, 반품 요청으로 이어지는 운영 흐름에 집중할 수 있었습니다.
...
일반 API 요청 중 401이 발생했을 때 서버와 프론트가 무한히 refresh/retry를 반복하지 않도록 역할을 나눴습니다.
...
비밀번호 재설정 요청과 확인 API 흐름은 구현되어 있습니다.

다만 이메일 발송 서비스 연동은 프로젝트 범위에서 제외했습니다.  
개발 환경에서는 재설정 토큰을 서버 콘솔 출력으로 확인하는 방식입니다.
...
프론트엔드에서 응답 구조를 일관되게 처리할 수 있도록 성공 응답은 `{ data }` 기준으로 변환합니다.
...
비회원은 체크아웃 화면에서 입력한 주문자 정보와 배송 정보로 주문을 생성합니다.
...
서비스 안에서는 배송 상태를 `SHIPPED`, `DELIVERED`처럼 변경하는 흐름에 집중했습니다.
...
관리자 수정 API는 현재 FAQ 정책 수정만 사용합니다.
